    ...
    if(check_alphabet(type) == 0)
    {
        ...
        else
        {
            switch(*type)
            {
                case 'c' :
                    printing_board(row, column, display) ;
                    select_column(row, column, display, found, arr) ;
                    break ;
                case 'u' :
                    undo(row, column, display, found, arr) ;
                    break ;
                case 'r' :
                    redo(row, column, display, found, arr) ;
                    break ;
                case 's' :
...
void select_column(int row, int column, char display[row][column], int found, int arr[row*column*2])
{
...
void select_column_comp(int row, int column, char display[row][column], int found, int arr[row*column*2])
{
    int number_of_column, i ;
    char number_of_col[1000] ;
    ...
    if (check_alphabet(number_of_col) == 1)
    {
        ...
        if (number_of_column > column || number_of_column < 1)
        {
            ...
            reset() ;
            printf("please try again and choose column from 1 to %d\n", column) ;
            select_column_comp(row, column, display, found, arr) ;
        }
        else
        {
            n_undo_redo = 0 ;
...
//the start of flow
void game_flow (int row, int column, char display[row][column], int found, int arr[row*column*2])
{
    ...
    else if(found == 1)         // player 1 turn
    {
        ...
        found = 2 ;
        game_flow(row, column, display, found, arr) ;
    }
